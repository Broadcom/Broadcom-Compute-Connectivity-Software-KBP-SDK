...
struct op2_xpt
{
    ...
    /**
     * API to acquire mutex lock which is used in device manager level
     *
     * @param handle Opaque OP2 user handle.
     *
     * @return KBP_OK on success else an error code
    ...
    kbp_status (*op2_mutex_lock)(void *handle);

    /**
     * API to release mutex lock which is used in device manager level
     *
     * @param handle Opaque OP2 user handle.
     *
     * @return KBP_OK on success else an error code
    ...
    kbp_status (*op2_mutex_unlock)(void *handle);
};
