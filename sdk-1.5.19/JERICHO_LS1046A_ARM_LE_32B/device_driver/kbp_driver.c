...
#define KBP_DRV_READ_PCIE_REG(device, reg, value) \
    ((value) = ioread32(device->regmap_virt + (reg)));

#define KBP_DRV_WRITE_PCIE_REG(device, reg, value) \
...
struct kbp_device {
    ...
    struct task_struct *int_owner_task;
...
#ifdef INT_DEBUG
        KBP_INFO("KBP Step1:Signal Number = %d, Owner task = %p Interrupt Owner task = %p \n", signal_num, device->owner_task, device->int_owner_task);
#endif
        ret = send_sig_info(signal_num, &si, device->int_owner_task);
    } else {
#ifdef INT_DEBUG
        KBP_INFO("KBP Step2:Signal Number = %d, Owner task = %p Interrupt Owner task = %p \n", device->signal_num, device->owner_task, device->int_owner_task);
#endif
        ret = send_sig(device->signal_num, device->int_owner_task, 0);
    }
...
static int kbp_pci_probe(struct pci_dev *kbp_dev, const struct pci_device_id *kbp_id)
{
     ...
     * 1: OP, 2: OP2
     */
    KBP_DRV_READ_PCIE_REG(device, opb_pdc_registers_DEV_ID, regval);
                               ...
                               uint32_t address, char *name)
{
    ...
    KBP_DRV_READ_PCIE_REG(device, address, regval);
...
static void log_op2_pdc_registers(struct seq_file *m, struct kbp_device *dev)
{
    ...
    for (chl_id = 0; chl_id < dev->num_channels; chl_id++) {
        ...
        KBP_DRV_WRITE_PCIE_REG(dev, opb_pdc_registers_TX_CH_ID, regval);
    ...
    kbp_driver_log_reg(m, dev,  opb_pdc_registers_SAT_ADDR, "opb_pdc_registers_SAT_ADDR");
    kbp_driver_log_reg(m, dev,  opb_pdc_registers_SAT_DATA, "opb_pdc_registers_SAT_DATA");
}
...
static void log_op_pdc_registers(struct seq_file *m, struct kbp_device *dev)
{
    ...
    kbp_driver_log_reg(m, dev, icf_pdc_registers_SAT_ADDR, "icf_pdc_registers_SAT_ADDR");
    kbp_driver_log_reg(m, dev, icf_pdc_registers_SAT_DATA, "icf_pdc_registers_SAT_DATA");
}
...
#ifdef INT_DEBUG
            KBP_INFO(":Owner pid = 0x%x, Owner task = %p \n", current->pid, current);
#endif
...
static long setup_interrupts(struct kbp_device *dev, unsigned int signal_num)
{
    ...
    dev->int_owner_task = current;
#ifdef INT_DEBUG
    KBP_INFO(": Interrupt Owner task = %p \n", current);
#endif
...
static int dma_enable_disable(struct kbp_device *device, unsigned int enable)
{
    ...
    if (device_type == OP2 && !device->is_fpga) {
        ...
        KBP_DRV_READ_PCIE_REG(device, opb_pdc_registers_DMA_CONTROL, regval);
        SET_FIELD(regval, opb_pdc_registers, DMA_CONTROL, rx_dma_enable, enable);
        KBP_DRV_WRITE_PCIE_REG(device, opb_pdc_registers_DMA_CONTROL, regval);
        KBP_INFO(": Request DMA_CONTROL = %x for %s\n", regval, device->name);

        for (chl_id = 0; chl_id < device->num_channels; chl_id++) {
            /* Program Tx Channel ID register, Before program any Tx Registers */
            SET_FIELD(regval, opb_pdc_registers, TX_CH_ID, tx_channel_id, chl_id);
            KBP_DRV_WRITE_PCIE_REG(device, opb_pdc_registers_TX_CH_ID, regval);

            /* Enable/Disable Tx DMA */
            KBP_DRV_READ_PCIE_REG(device, opb_pdc_registers_RSP_Q_CTRL, regval);
            SET_FIELD(regval, opb_pdc_registers, RSP_Q_CTRL, tx_dma_enable, enable);
            KBP_DRV_WRITE_PCIE_REG(device, opb_pdc_registers_RSP_Q_CTRL, regval);
            KBP_INFO(": Response DMA_CONTROL = %x for %s\n", regval, device->name);
        }
    } else {
        KBP_DRV_READ_PCIE_REG(device, icf_pdc_registers_DMA_CONTROL, regval);
        ...
        KBP_DRV_WRITE_PCIE_REG(device, icf_pdc_registers_DMA_CONTROL, regval);
...
static int loopback_enable_disable(struct kbp_device *device, unsigned int enable_bmp)
{
    ...
    if (device_type == OP2 && !device->is_fpga) {
        KBP_DRV_READ_PCIE_REG(device, opb_pdc_registers_TEST_CAPABILITIES, regval);
        ...
        KBP_DRV_WRITE_PCIE_REG(device, opb_pdc_registers_TEST_CAPABILITIES, regval);
        KBP_DRV_READ_PCIE_REG(device, opb_pdc_registers_TEST_CAPABILITIES, regval);
    } else {
        KBP_DRV_READ_PCIE_REG(device, icf_pdc_registers_TEST_CAPABILITIES, regval);
        SET_FIELD(regval, icf_pdc_registers, TEST_CAPABILITIES, reqf_rspf_loopback, enable);
        KBP_DRV_WRITE_PCIE_REG(device, icf_pdc_registers_TEST_CAPABILITIES, regval);
        KBP_DRV_READ_PCIE_REG(device, icf_pdc_registers_TEST_CAPABILITIES, regval);
    }
...
static int kbp_initialize_dma(struct kbp_device *device)
{
    ...
    /* Clear interrupts */
    if (device_type == OP2 && !device->is_fpga) {
        KBP_DRV_WRITE_PCIE_REG(device, opb_pdc_registers_INTR_CLEAR, 0x1F07FFFF);
    } else {
        KBP_DRV_WRITE_PCIE_REG(device, icf_pdc_registers_INTR_CLEAR, 0xFFFFFFFF);
    }
    ...
    KBP_DRV_WRITE_PCIE_REG(device, icf_pdc_registers_REQ_Q_LBASE, regval);

    regval = 0;
    SET_FIELD(regval, icf_pdc_registers, REQ_Q_HBASE,
              address, (phy_addr >> 32) & 0xFFFFFFFF);
    KBP_DRV_WRITE_PCIE_REG(device, icf_pdc_registers_REQ_Q_HBASE, regval);
    ...
    for (chl_id = 0; chl_id < device->num_channels; chl_id++) {
        ...
        SET_FIELD(regval, icf_pdc_registers, RSP_Q_LBASE,
                  address, phy_addr & 0xFFFFFFFF);
        KBP_DRV_WRITE_PCIE_REG(device, icf_pdc_registers_RSP_Q_LBASE, regval);

        regval = 0;
        SET_FIELD(regval, icf_pdc_registers, RSP_Q_HBASE,
                  address, (phy_addr >> 32) & 0xFFFFFFFF);
        KBP_DRV_WRITE_PCIE_REG(device, icf_pdc_registers_RSP_Q_HBASE, regval);
    ...
    SET_FIELD(regval, icf_pdc_registers, REQ_H_LBASE,
              address, phy_addr & 0xFFFFFFFF);
    KBP_DRV_WRITE_PCIE_REG(device, icf_pdc_registers_REQ_H_LBASE, regval);

    regval = 0;
    SET_FIELD(regval, icf_pdc_registers, REQ_H_HBASE,
              address, (phy_addr >> 32) & 0xFFFFFFFF);
    KBP_DRV_WRITE_PCIE_REG(device, icf_pdc_registers_REQ_H_HBASE, regval);
    ...
    SET_FIELD(regval, icf_pdc_registers, REQ_T_LBASE,
              address, phy_addr & 0xFFFFFFFF);
    KBP_DRV_WRITE_PCIE_REG(device, icf_pdc_registers_REQ_T_LBASE, regval);

    regval = 0;
    SET_FIELD(regval, icf_pdc_registers, REQ_T_HBASE,
              address, (phy_addr >> 32) & 0xFFFFFFFF);
    KBP_DRV_WRITE_PCIE_REG(device, icf_pdc_registers_REQ_T_HBASE, regval);
    ...
    for (chl_id = 0; chl_id < device->num_channels; chl_id++) {
        ...
        SET_FIELD(regval, icf_pdc_registers, RSP_H_LBASE,
                  address, phy_addr & 0xFFFFFFFF);
        KBP_DRV_WRITE_PCIE_REG(device, icf_pdc_registers_RSP_H_LBASE, regval);

        regval = 0;
        SET_FIELD(regval, icf_pdc_registers, RSP_H_HBASE,
                  address, (phy_addr >> 32) & 0xFFFFFFFF);
        KBP_DRV_WRITE_PCIE_REG(device, icf_pdc_registers_RSP_H_HBASE, regval);
    ...
    for (chl_id = 0; chl_id < device->num_channels; chl_id++) {
        ...
        SET_FIELD(regval, icf_pdc_registers, RSP_T_LBASE,
                  address, phy_addr & 0xFFFFFFFF);
        KBP_DRV_WRITE_PCIE_REG(device, icf_pdc_registers_RSP_T_LBASE, regval);

        regval = 0;
        SET_FIELD(regval, icf_pdc_registers, RSP_T_HBASE,
                  address, (phy_addr >> 32) & 0xFFFFFFFF);
        KBP_DRV_WRITE_PCIE_REG(device, icf_pdc_registers_RSP_T_HBASE, regval);
    ...
    KBP_DRV_WRITE_PCIE_REG(device, icf_pdc_registers_SAT_M_TIMER_LO, sat_timer_lo);
    KBP_DRV_WRITE_PCIE_REG(device, icf_pdc_registers_SAT_M_TIMER_HI, sat_timer_hi);
