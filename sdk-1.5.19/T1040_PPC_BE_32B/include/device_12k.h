...
#ifndef __DEVICE_12K_H
#define __DEVICE_12K_H
...
#define KBP_DEV_12K_DEVICE_CONFIG_REG_SUPPRESS_INDEX_ENABLE_BIT      (30)
...
/*x is the parallel search number*/
...
/* Bit positions in 32b result value */
...
struct kbp_lut_rslt_IdxADCfg {
    uint8_t m_numBytes;          /**< number of bytes to be transferred in Index/Asso-Data
...
enum kbp_dev_12k_write_mode {
    KBP_DEV_12K_DM = 0,  /**< Write is in data/mask format */
...
struct kbp_dev_12k_err_status_reg {
    ...
    int32_t m_CRxBnk0FifoOverflowErr;      /**< 62: R/clear */
//  int32_t m_CRxBnk1FifoOverflowErr;      /**< 62: R/clear */
    ...
    /* Packet errors */
    int32_t m_OM_ROP_Err;                  /**< 47: */
    int32_t m_IB_PIPE_RBD_FIFO_Parity_Err; /**< 46: */
    int32_t m_UD_2b_ECC_Err;               /**< 45: */
    int32_t m_RB_Ctxid_Inst_Mismatch_Err;  /**< 44: */
    ...
    int32_t m_PowerLimitingErr;            /**< 38: */
    int32_t m_ctxBuffParityErr;            /**< 37: R/clear */
    int32_t m_ltrParityErr;                /**< 36: R/clear */
    int32_t m_devIdMisMatchErr;            /**< 35: R/clear */
    int32_t m_illegalInstErr;              /**< 34: R/clear */
    ...
    int32_t m_IB_PIPE_FIFO_UnderflowErr;   /**< 12: */
    int32_t m_IB_PIPE_FIFO_OverflowErr;    /**< 11: */
...
struct kbp_dev_12k_db_config_reg {
    ...
    int32_t m_suppressIdxOnAD;   /**< 30  if set, index will be suppressed.
...
struct kbp_dev_12k_shadow_ltr {
    ...
    struct kbp_dev_12k_ext_cap0_reg m_extCap0; /* m_miscellaneous */
 ...
 *
 * @param device Valid device handle.
 * @param devNum device number (represent device number if cascaded devices).
 * @param abNum  array block number.
 * @param abAddr MSB row number.
 * @param data valid data pointer to write (which will written to device as chunks of 80b from MSB to LSB)
 * @param mask valid data pointer to write (which will written to device as chunks of 80b from MSB to LSB)
 ...
 * @param parity which holds the parity error occurred or not
 ...
 * @param abNum  array block number.
 * @param abAddr MSB row number.
 * @param o_data valid data pointer to read (which will be read from device as chunks of 80b from MSB to LSB)
 * @param o_mask valid mask pointer to read (which will be read from device as chunks of 80b from MSB to LSB)
 ...
 * @param data valid associated data (which will be written to device as chunks of 32b from MSB to LSB)
 ...
 * @param cbInfo Contains the valid parameters like, masterkey, width, context addressvalid address for search
 * @param o_search_results Structure which holds the results
 *                  (VBit, SMT, ADWidth, HitIdx, Asso-data) for up to 6 results
 *
 * @return KBP_OK on success or an error code otherwise.
 */
 ...
 *   (Note: can issue NetRoute only on result port-0
 *       128 sets of LTR, 4 keys, and 4 result ports are available.
 *       Single Bank mode:
 *           0-127 for BANK 0, 4 keys, and 4 result ports.
 *
 ...
 *   Implements a special search instruction. It searches the input key into the
 *   database depending on LTR profile set number. This instruction can be used for searches
 *   with individual search key lengths of up to 640 bits. This API used to get up to 6 results
 *
 *   Compare 3 makes use of 2 LTR profiles, first LTR will return up to 4 results and the LTR+1
 *   will return the 2 results; LTRn key2/3 must of > 320b
 *   NetRoute can be searched only on result port 0 or 2
 *   LTR wrap will happen from 127 to 0 (non SMT), 63 - 0: 127 - 64 (SMT mode)
 *
 * @param device Valid device handle.
 * @param ltrNum LTR number for the search
 * @param cbInfo Contains the valid parameters like, masterkey, width, context addressvalid address for search
...
/**
 *   Flat 80b generic read from the specified address (mainly used for 80b reg reads along with parity bit)
 *
 * @param device Valid device handle.
 * @param devNum device number
 * @param address valid address to read
 * @param o_data array of 10B which holds the read data
 * @param o_parity return the parity bit (set of parity error, else 0)
 * @return KBP_OK on success or an error code otherwise.
 */
kbp_status kbp_dm_12k_generic_reg_read_with_parity(struct kbp_device *device, uint8_t devNum,
                                                   uint32_t address, void *o_data, uint32_t *o_parity);

/**
 *   Handling of special NetRoute instructions (Type1/2/3).
 *
 * @param device Valid device handle.
 * @param devNum device number
 ...
 * @param data Valid data to be written for the above 3 types of operations
...
/**
 *   Perform NOP operation for specified number of times
 *
 * @param device Valid device handle.
 * @param devNum device number
...
/**
 *   Performs the supported Block operations, these can be Block 1. Move 2. Clear 3. Copy
...
/**
 * This API used to inject the parity errors by programming the NETL debug register (0x7FE, 0x7FF).
 * as of now we are corrupting both X/Y [D/M] rows by flipping the X: b:1 and Y: b:5,4, and 2 of 80b
 ...
 * @param fp File pointer to print the data to console or to file (default console)
 *
 * @return KBP_OK on success or an error code otherwise.
 */
