...
/* For 80-bit registers, the register addresses below use internal offset (80-bit space) and not byte offset
 * So e.g. if 80-bit register A has internal offset of 0xC01C, the next 80-bit register B in the sequence
 * will have internal offset of 0xC01D and not 0xC026 */
/** HB Power up */
#define KBP_OP_HB_POWER(is_op2)                   (is_op2 ? 0x000C00D : 0x000C00C)

/** DBA power up for ABs 0-127 */
#define KBP_OP_REG_DBA_PWR_0                      0x00008010

/** DBA power up for ABs 127-255 */
#define KBP_OP_REG_DBA_PWR_1                      0x00008011

/** DBA power up status reg for ABs 0-127 */
#define KBP_OP_REG_DBA_PWR_STS_0                  0x00008020

/** DBA power up status reg for ABs 127-255 */
#define KBP_OP_REG_DBA_PWR_STS_1                  0x00008021

/** UDA power up for first 32 UDMs */
#define KBP_OP_REG_UDA_PWR_0                      0x0000C01C

/** UDA power up for last 32 UDMs */
#define KBP_OP_REG_UDA_PWR_1                      0x0000C01D

/** UDA power up status reg for first 32 UDMs */
#define KBP_OP_REG_UDA_PWR_STS_0(is_op2)          (is_op2 ? 0x0000C020 : 0x0000C01E)

/** UDA power up status reg for last 32 UDMs */
#define KBP_OP_REG_UDA_PWR_STS_1(is_op2)          (is_op2 ? 0x0000C021 : 0x0000C01F)

/** Status register read poll count */
#define KBP_OP_STATUS_REG_POLL_COUNT              1000
...
struct op_dev_dba_fifo_reg {
    __EO_6(uint32_t location:20,                      /**< 19:0  location */
           uint32_t reserved1:10,                     /**< 29:20 reserved, write 0s; on read unknown values */
...
struct op_dev_uda_fifo_reg {
    __EO_9(uint32_t location:23,                     /**< 22:0  location */
           uint32_t reserved1:5,                     /**< 27:23 reserved, write 0s; on read unknown values */
           ...
           uint32_t reserved2:1,                     /**< 31    reserved, write 0s; on read unknown values */
           uint32_t err_addrs_valid:1,               /**< 32    error address is valid or not */
           uint32_t clear_fifo:1,                    /**< 33    Writing 1 will flush the fifo. It's self clearing; read always returns 1'b0. */
...
struct op_dev_fifo_status_reg {
    __EO_10(uint32_t fifo_rd_ptr:5,                       /**<  4:0  FIFO read pointer; ECC error fifo read pointer */
            uint32_t reserved1:3,                         /**<  7:5  reserved, write 0s; on read unknown values */
            uint32_t fifo_wr_ptr:5,                       /**< 12:8  FIFO write pointer; ECC error fifo write pointer */
            uint32_t reserved2:3,                         /**< 15:13 reserved, write 0s; on read unknown values */
            uint32_t fifo_entry_cnt:6,                    /**< 21:16 FIFO count; number of entries in ECC FIFO */
            uint32_t reserved3:2,                         /**< 23:22 reserved, write 0s; on read unknown values */
            uint32_t fifo_overflow:1,                     /**< 24    overflow on ECC error FIFO */
            uint32_t fifo_full:1,                         /**< 25    ECC error FIFO full */
            uint32_t reserved4:6,                         /**< 31:26 reserved, write 0s; on read unknown values */
            uint32_t reserved5);                          /**< 63:32 Reserved5 */
};

/*
 * Defines addresses in the Optimus Prime register space (direct address space)
 */

#define KBP_OP2_ERROR_EMULATION_CORE_REG            0x00000144
#define KBP_OP2_ERROR_EMULATION_INTF_REG            0x00000148
...
#define KBP_OP_DEVICE_ERROR_STATUS_CORE0            0x00000002
#define KBP_OP_DEVICE_ERROR_STATUS_CORE1            0x00000003
...
#define KBP_OP__MDIO__ERROR_STATUS_PORT0           0x00000008
#define KBP_OP__MDIO__ERROR_STATUS_PORT1           0x0000000C


/** Address of the interrupt enable register port#0 and port#1 */
#define KBP_OP_DEVICE_INTR_MASK_ENABLE_PORT0        0x00000004
#define KBP_OP_DEVICE_INTR_MASK_ENABLE_PORT1        0x00000005

/** Address of the interface error status */
#define KBP_OP2_DEVICE_INTF_ERROR_STATUS            0x00000004
#define KBP_OP2__MDIO__INTF_ERROR_STATUS            0x00000010
...
/** Address of the error enable register */
#define KBP_OP2_DEVICE_ERR_ENABLE_PORT              0x00000007

/** Address of the interrupt mask enable register of ARM CPU and PCIE */
#define KBP_OP_DEVICE_INTR_MASK_ENABLE_ARM_CPU      0x00000008
#define KBP_OP_DEVICE_INTR_MASK_ENABLE_PCIE         0x00000009

/** Address of the parity scan interval register */
#define KBP_OP_DEVICE_PAR_INTV_REG                  0x0000000A

/** Address of the ARM EMC Status register */
#define KBP_OP_DEVICE_ARM_EMC_STATUS_REG            0x0000000B
...
#define KBP_OP_LTR_ADDR_BLOCK_SELECT(is_op2, ltr, bl_64)    (is_op2 ? (0x00040030 + (ltr) * 0x100 + (bl_64)) : (KBP_OP_REG_LTR_START + (ltr) * 0x100 + (bl_64)))
/** Result map -- 4 bits per array block, 16 array blocks per register */
#define KBP_OP_LTR_ADDR_PARALLEL_SEARCH(is_op2, ltr, bl_16) (is_op2 ? (0x00040040 + (ltr) * 0x100 + (bl_16)) : (0x00040010 + (ltr) * 0x100 + (bl_16)))
...
#define KBP_OP_LTR_ADDR_KPU_0(is_op2, ltr, kpu)     (is_op2 ? (0x0004000C + (ltr) * 0x100 + (kpu) * 3) : (0x0004005C + (ltr) * 0x100 + (kpu) * 2))
/** Second logical table register for given KPU */
#define KBP_OP_LTR_ADDR_KPU_1(is_op2, ltr, kpu)     (is_op2 ? (0x0004000D + (ltr) * 0x100 + (kpu) * 3) : (0x0004005D + (ltr) * 0x100 + (kpu) * 2))
/** Range extraction */
#define KBP_OP_LTR_ADDR_RANGE_EXT(is_op2, ltr, rng_6)       (is_op2 ? (KBP_OP_REG_LTR_START + (ltr) * 0x100 + (rng_6)) : (0x00040070 + (ltr) * 0x100 + (rng_6)))
/** Post-KPU key processing data for output of given KPU */
#define KBP_OP_LTR_ADDR_PKP(is_op2, ltr, kpu)       (is_op2 ? (0x00040020 + (ltr) * 0x100 + (kpu)) : (0x00040074 + (ltr) * 0x100 + (kpu)))
/** Logical table register defining the widths of UDA keys */
#define KBP_OP_LTR_ADDR_UDA_KEY(is_op2, ltr)        (is_op2 ? (0x0004001E + (ltr) * 0x100) : (0x00040080 + (ltr) * 0x100))
/** Search attribute register */
#define KBP_OP_LTR_ADDR_SRCH_ATTR(is_op2, ltr)      (is_op2 ? (0x00040080 + (ltr) * 0x100) : (0x00040082 + (ltr) * 0x100))
/** AD hit bit configuration register */
#define KBP_OP_LTR_ADDR_AD_HB(is_op2, ltr)          (is_op2 ? (0x00040081 + (ltr) * 0x100) : (0x00040083 + (ltr) * 0x100))
/** Logical table register controlling result generation including AD */
#define KBP_OP_LTR_ADDR_AD_RES(is_op2, ltr)         (is_op2 ? (0x00040082 + (ltr) * 0x100) : (0x00040084 + (ltr) * 0x100))
/** Search-n-Count (SnC) configuration register. */
#define KBP_OP2_LTR_ADDR_SNC(ltr_idx)               (0x00040083 + (ltr_idx) * 0x100)
/** Port Q Control (QCTRL) register address. */
#define KBP_OP2_ADDR_PORT_QCTRL_START               (0x000400F1)
#define KBP_OP2_ADDR_PORT_QCTRL_END                 (0x00047FF1)
#define KBP_OP2_LTR_ADDR_PORT_QCTRL(ltr_idx)        (0x000400F1 + (ltr_idx) * 0x100)
#define KBP_OP2_ADDR_PORT_QCTRL_MASK                (0x000700FF)
/** One control register for each UDC */
#define KBP_OP_LTR_ADDR_UDC(is_op2, ltr, udc)       (is_op2 ? (0x000400a0 + (ltr) * 0x100 + (udc)) : (0x00040090 + (ltr) * 0x100 + (udc)))
/** Conflict bitmap */
#define KBP_OP_LTR_ADDR_CONFLICT_BMP(core_id, ltr)  (0x000400F0 + ((core_id * 128) + ltr) * 0x100)
/** End of address space for LTR-related registers */
#define KBP_OP_REG_LTR_END                              (0x00047FFF)
#define KBP_OP2_REG_LTR_END                             (0x0004FFFF)
...
/** Start of address space for context buffer : [16:0] = 0x140000
    Dual port   : {P0: 0-0x4FFF, P1: 0x5000-0x9FFF}, {P2: 0xA000-0xEFFF, P3: 0xF000-0x13FFF}
    Quad port   :  P0: 0-0x4FFF, P1: 0x5000-0x9FFF,   P2: 0xA000-0xEFFF, P3: 0xF000-0x13FFF
*/
#define KBP_OP2_CB_DEPTH                             0x00014000
#define KBP_OP2_REG_CB_START                         0x00120000
/** End of address space for context buffer */
#define KBP_OP2_REG_CB_END                           0x00133FFF
#define KBP_OP2_ADDR_CONTEXT_BUFFER(x)               (0x00120000 + (x))
...
#define KBP_OP2_EMC_REGS_END                        0x83a
...
#define KBP_OP2_UDM_SEL_TABLE_SIZE                                  48
#define KBP_OP2_REG_ADDR_UDR_UDM_SEL_TABLE_START                    0xC100
#define KBP_OP2_REG_ADDR_UDR_UDM_SEL_TABLE_END                      0xC160
#define KBP_OP2_REG_ADDR_UDR_UDM_SEL_TABLE(index, part_no)          (KBP_OP2_REG_ADDR_UDR_UDM_SEL_TABLE_START + (index * 2) + part_no)

#define KBP_OP2_AD_TRANS_CAM_SIZE                                   16
#define KBP_OP2_REG_ADDR_UDR_AD_TRANS_TABLE_START                   0xCC00
#define KBP_OP2_REG_ADDR_UDR_AD_TRANS_TABLE_END                     0xCFCF
#define KBP_OP2_REG_ADDR_UDR_AD_TRANS_TABLE(udc_index, cam_index)   (KBP_OP2_REG_ADDR_UDR_AD_TRANS_TABLE_START | (cam_index << 6) | udc_index)

#define KBP_OP2_AGING_TRANS_CAM_SIZE                                16
#define KBP_OP2_REG_ADDR_UDR_AGING_TRANS_TABLE_START                0xC300
#define KBP_OP2_REG_ADDR_UDR_AGING_TRANS_TABLE_END                  0xC30F
#define KBP_OP2_REG_ADDR_UDR_AGING_TRANS_TABLE(cam_index)           (KBP_OP2_REG_ADDR_UDR_AD_TRANS_TABLE_START + cam_index)

enum kbp_dev_op_parity_inject_type {
    KBP_OP_DISABLE_ERR_INJECT = 0,

    KBP_OP_DBA_ERR_INJECT = 1, /* for DBA */
    KBP_OP_UDA_ERR_INJECT = 2, /* associated data or advance LSN instructions */
    KBP_OP_UIT_ERR_INJECT = 4, /* for UDR/UIT */
    KBP_OP_RPT_ERR_INJECT = 8, /* for Advance DBA (RPT)*/
    KBP_OP_ALL_ERR_INJECT = 16,/* generic one for all errors above */
    KBP_OP_ERR_INJECT_END
};

/** OP/OP2 LTR registers types. */
enum kbp_dev_op_ltr_reg_type {
    ...
    KBP_OP_LTR_PARALLEL_SEARCH_5_REG = 25,
    ...
    KBP_OP_LTR_PARALLEL_SEARCH_10_REG = 30,
    ...
    KBP_OP_LTR_PARALLEL_SEARCH_15_REG = 35,
    /** Logical table register controlling result generation including AD */
    KBP_OP_LTR_AD_RESULT_REG = 36,
    /** Miscellaneous Registers */
    KBP_OP_LTR_MISC1_REG = 37,
    KBP_OP_LTR_MISC2_REG,
    KBP_OP_LTR_MISC3_REG,
    KBP_OP_LTR_MISC4_REG,
    KBP_OP_LTR_PKP_0_REG = 41,
    KBP_OP_LTR_PKP_1_REG,
    KBP_OP_LTR_PKP_2_REG,
    KBP_OP_LTR_PKP_3_REG,
    KBP_OP_LTR_MISC6_0_REG = 45,
    KBP_OP_LTR_MISC6_1_REG,
    KBP_OP_LTR_MISC6_2_REG,
    KBP_OP_LTR_MISC6_3_REG,
    KBP_OP_LTR_MISC6_4_REG,
    KBP_OP_LTR_MISC6_5_REG = 50,
    KBP_OP_LTR_MISC6_6_REG,
    KBP_OP_LTR_MISC6_7_REG,
    KBP_OP_LTR_MISC6_8_REG,
    KBP_OP_LTR_MISC6_9_REG = 54,
    KBP_OP_LTR_MISC6_10_REG = 55,
    KBP_OP_LTR_MISC6_11_REG,
    KBP_OP_LTR_MISC6_12_REG,
    KBP_OP_LTR_MISC6_13_REG,
    KBP_OP_LTR_MISC6_14_REG,
    KBP_OP_LTR_MISC6_15_REG,
    KBP_OP_LTR_MISC7_REG = 61,
    KBP_OP_LTR_REG_END = 62,
    KBP_OP2_LTR_SNC_CTRL_REG = 63,
    /** KCR-2 logical table register for given KPU (OP2-Specific) */
    KBP_OP2_LTR_KPU0_KCR2_REG = 64,
    KBP_OP2_LTR_KPU1_KCR2_REG,
    KBP_OP2_LTR_KPU2_KCR2_REG,
    KBP_OP2_LTR_KPU3_KCR2_REG,
    KBP_OP2_LTR_RES_CTRL_REG = 68,
    /** Register end value */
    KBP_OP2_LTR_REG_END
};
...
struct op_shadow_ltr {
    enum kbp_dev_op_ltr_reg_type reg_type;
    ...
    struct op2_kpu_segs_a_e kpu_0_2;  /**< 60 bits controlling parsing */
    ...
    struct op2_kpu_segs_a_e kpu_1_2;  /**< 60 bits controlling parsing */
    ...
    struct op2_kpu_segs_a_e kpu_2_2;  /**< 60 bits controlling parsing */
    ...
    struct op2_kpu_segs_a_e kpu_3_2;  /**< 60 bits controlling parsing */
    struct op_range_ext r_e[3];       /**< 2 60-bit and one 40-bit registers controlling 16 range units total */
    struct op2_res_misc_ctrl op2_res_ctrl;
    ...
    uint64_t srch_n_cnt;              /**< A 64-bit shadow struct op2_srch_cnt_ctrl register. */
};
...
/**
 * Write to specific device/core only
 */
kbp_status kbp_dm_op_dba_write_row_to_device(struct kbp_device * device, int32_t blk_num, int32_t entry_nr,
                                             uint8_t *data, uint8_t *mask, uint8_t is_xy, uint32_t valid_bit);
 ...
 * @return KBP_OK on success or an error code otherwise.
 */
kbp_status kbp_dm_op_uda_write(struct kbp_device *device, uint32_t address, uint8_t *data, uint8_t length, uint32_t algo_ad);

/**
 *   Writes a stream of associated data (must be 32b/64b/128b/256b)to the specified UDA address,
 *   the associated data written from MSB to LSB, so udaAddr has to be MSB address
 *   this is regular AD write API
 *
 * @param device Valid device handle.
 * @param address MSB UDA address location.
 * @param data Valid associated data
 ...
 * @return KBP_OK on success or an error code otherwise.
 */
kbp_status kbp_dm_op_uda_write_as_pio(struct kbp_device *bc_device, uint32_t uda_address, uint8_t *data, uint8_t length_8);

/**
 *   Writes a stream of associated data (must be 32b/64b/128b/256b)to the specified UDA address,
 *   the associated data written from MSB to LSB, so udaAddr has to be MSB address
 *   this is regular AD write API, and writes to specific device only which passed as 1st arg.
 *
 * @param device Valid device handle.
 * @param address MSB UDA address location.
 * @param data Valid associated data
 ...
 * @return KBP_OK on success or an error code otherwise.
 */
kbp_status kbp_dm_op_uda_pio_write_to_device(struct kbp_device *device, uint32_t address, uint8_t *data, uint8_t length_8);
 ...
 * @return KBP_OK on success or an error code otherwise.
 */
kbp_status kbp_dm_op_uda_read(struct kbp_device *device, uint32_t address, uint8_t length, uint8_t *o_data);
 ...
 * @return KBP_OK on success or an error code otherwise.
 */
kbp_status kbp_dm_op_command_send(struct kbp_device *device, uint32_t opcode, uint32_t length, uint8_t *bytes);

/**
 *   Performs the Operations, these can be Block and LSN operations, and writes to specific
 *   device/core only
 *
 * @param device Valid device handle.
 * @param opcode Operation code
 ...
 * @return KBP_OK on success or an error code otherwise.
 */
kbp_status kbp_dm_op_command_send_to_device(struct kbp_device *bc_device, uint32_t opcode, uint32_t length, uint8_t * bytes);
 ...
 * Write to specific device/core only
 */
kbp_status kbp_dm_op_reg_write_to_device(struct kbp_device * device, uint32_t address, uint8_t *data);
...
kbp_status kbp_dm_op_inject_parity(struct kbp_device *device, enum kbp_dev_op_parity_inject_type inject_type);
...
kbp_status kbp_dm_op_clear_errors(struct kbp_device *device);

/**
 * Clear the Error status registers OP2 (#port0 and #port)
 *
 * @param device Valid device handle
...
kbp_status kbp_dm_op2_clear_errors(struct kbp_device *device, struct kbp_status_errors *sel_clr);

/**
 * Enable/Disable DBA Parity Scan, Currently supported for OP2 only.
 *
 * @param device Valid device handle
 * @param enable Flag to enable/disable DBA parity scan. By default parity scan is enabled
 *
 * @return KBP_OK on success or an error code otherwise
 */

kbp_status kbp_device_dba_parity_scan(struct kbp_device *device, uint8_t enable);
