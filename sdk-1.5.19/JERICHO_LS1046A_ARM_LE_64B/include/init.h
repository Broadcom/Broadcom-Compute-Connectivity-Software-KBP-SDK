...
/**
 * Supported Max Optimus Prime 2 lanes
 */

#define KBP_MAX_SEARCH_INTF_LANES   32

/**
 * Supported Max KBP Device Ports
 */

#define KBP_DEVICE_MAX_PORTS        4
...
struct kbp_device_interface_settings {
    ...
    struct kbp_serdes_tap_settings per_lane_tap_settings[KBP_MAX_SEARCH_INTF_LANES];
    uint16_t user_tap_settings; /** 0=Default Settings to all lanes,1=Same tap_settings to all lanes,2=per_lane_tap_settings */
...
struct kbp_device_config {
    ...
    } port_map[KBP_DEVICE_MAX_PORTS]; /**< The flags argument to kbp_device_init() or kbp_device_interface_init() must be OR'd with KBP_DEVICE_DUAL_PORT, to honor second port info. */
    ...
    int16_t tx_rop_pkt_timeout; /* Tx ROP Packetization timeout value */
};
 ...
 * Apply System reset
 *
 * @param type The device type, currently Optimus Prime, Optimus Prime 2 & TAP are supported
 * @param config Interface configuration structure initialized correctly
 *
 */
...
/**
 * API to Clear device interface errors which are addressed by SDK
 *
 * @param type The device type, currently Optimus Prime and Optimus Prime2 are supported
 * @param config Configuration structure initialized appropriately.
 * @param intf_clr_errs_bmp Clear Errors bitmap
 *
 * @return KBP_OK on success or an error code.
 */

kbp_status kbp_device_interface_clear_errors(enum kbp_device_type type, struct kbp_device_config *config,
                                             uint64_t intf_clr_errs_bmp);

/**
 * API to Program Tx ROP Packetization timeout value
 *
 * @param type The device type, Optimus Prime2 & OP2P are supported
 * @param config Configuration structure initialized appropriately
 * @param value Timeout value, valid 0 - 511
 *
 * @return KBP_OK on success or an error code.
 */
