...
/** Number of bits to right shift for Pointer Min and Max Programming for OP2 Stats */
#define KBP_OP2_STATS_PTR_SHIFT                             10
...
#define KBP_OP2_COUNTER_SCAN_BULK_READ_SIZE                 (32768)
...
#define KBP_OP2_TX_DMA_BUFFER_SIZE_COUNTER_BULK_READ        (128 * 1024)
...
/** Maximum number of threads possible in OP2 */
#define KBP_OP2_MAX_NUM_THREADS                             (4)
#define KBP_OP2_MAX_THREAD_NUM                              (3)

/** Offset to add to db IDs created on Th-2 & Th-3 */
#define KBP_OP2_TAP_DB_ID_OFFSET                            (32)
...
/**
 * API to perform write in Stats path, and writes to target device only
 *
 * @param device Valid device handle
 * @param address Valid device address
 * @param data Array which holds to data to write
 ...
 * @return KBP_OK on success or an error code on failure.
 */
kbp_status kbp_dm_op2_stats_pio_write_to_device(struct kbp_device *device, uint32_t address, uint8_t *data);
...
/**
 * API to perform write in Stats path to a particular core of OP2 device (and to the same core
 * of next device incase of OP2P)
 *
 * @param device Valid device handle
 * @param address Valid device address
 * @param data Array which holds to data to write
 ...
 * @return KBP_OK on success or an error code on failure.
 */
kbp_status kbp_dm_op2_tap_pio_write(struct kbp_device *device, uint32_t address, uint8_t *data);
...
/**
 * Acquire device mangaer level mutex lock
 *
 * @param device Valid device handle
 *
 * @return KBP_OK on success or an error code otherwise
 */
kbp_status kbp_dm_acquire_lock(struct kbp_device *device);

/**
 * Release device mangaer level mutex lock
 *
 * @param device Valid device handle
 *
 * @return KBP_OK on success or an error code otherwise
 */
kbp_status kbp_dm_release_lock(struct kbp_device *device);
